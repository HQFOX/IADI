...
import {Reviewer, ReviewerPanels} from "../types/Reviewer.type";
...
const defaultReviewer: ReviewerPanels = {
    reviewerId : 0,
    name : "",
    evaluationPanels: []
}
...
/**
 function loadGrants(setGrants:(grants:GrantCall[]) => void) {
    getData('/grantcall',[])
        .then(data => { data && setGrants(data as GrantCall[])})
}
 **/

function loadChairEvaluationPanels(setPanels:(panels:EvaluationPanel[]) => void, currentId:number) {
    getData('/panel/chair/' + currentId,[])
        .then(data => {
...
const ChairPage = () => {
    ...
    useEffect( () => {
        loadChairEvaluationPanels(setPanels, login.currentId)
    } , [])
